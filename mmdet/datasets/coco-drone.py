...
class CocoDataset(CustomDataset):
    ...
    def process_and_cluster_patches_with_bbox(self, images, bboxes):
        """
        根据目标范围提取包含Drone的patch并聚类。
        ...
        Returns:
            List[List[Tensor]]: 聚类后的patch列表，每个类对应一个patch列表
        """
        # 提取目标范围内的patch
        drone_patches = self.patch_extractor.extract_drone_patches_with_bbox(images, bboxes)

        # 聚类
        clustered_patches = self.patch_extractor.cluster_patches(drone_patches, self.num_clusters)
